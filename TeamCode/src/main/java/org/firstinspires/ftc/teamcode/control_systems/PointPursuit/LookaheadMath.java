...
// Parameter lookaheadDistance: The radius of the circle
    public static ArrayList<CurvePoint> lineCircleIntersection(CurvePoint linePoint1 , CurvePoint linePoint2 , CurvePoint robotLocation , double lookaheadDistance){
    ...
    */
    ...
    /*Now that we have the quadraticA , quadraticB , and quadraticC , we can plug these
     values into the quadratic formula to the get the x values of the intersection points*/
        ...
        ArrayList<CurvePoint> intersectionPoints = new ArrayList<>();
        ...
        */
            ...
            if(x1 > minX && x1 < maxX && y1 > minY && y1 < maxY){
                intersectionPoints.add(new CurvePoint(x1 , y1));
            }
            ...
            if(x2 > minX && x2 < maxX && y2 > minY && y2 < maxY){
                intersectionPoints.add(new CurvePoint(x2 , y2));
            }
    ...
    public static CurvePoint clipToLine(CurvePoint linePoint1 , CurvePoint linePoint2 , CurvePoint robotLocation){
...
// Parameter robotLocation : Pass in the robot location (x position , y position)
    public static CurvePoint clipToPath(ArrayList<CurvePoint> path , CurvePoint robotLocation){
        ...
        for(int i = 0 ; i < path.size() - 1; i ++){
            CurvePoint startPoint = path.get(i); // The start point of the line segment
            CurvePoint endPoint = path.get(i + 1); // The end point of the line segment

            // Intersect the perpendicular line
            CurvePoint clippedToLine = clipToLine(startPoint , endPoint , robotLocation);
...
// Parameter robotLocation : Pass in the robot location (x position , y position)
    // Parameter path : Pass in the path array list that the robot is following
// Parameter robotLocation : Pass in the robot location (x position , y position)
    public static CurvePoint getLookaheadPoint(ArrayList<CurvePoint> path , CurvePoint robotLocation , CurvePoint currIndexInPath){
    ...
    */
        CurvePoint currPointInPath = path.get(currIndexInPath.pointIndex); //clipToPath(path, robotLocation);
        // Start off by setting the lookahead point to something random so it isn't null
        CurvePoint lookahead = path.get(path.size() -1);
        // Store the lastPoint as a Point so we can use it later on in the method
        CurvePoint lastPoint = path.get(path.size() - 1);
        ...
        */
            for(CurvePoint intersection : intersections){
    ...
    public static ArrayList<CurvePoint> extendedPath(ArrayList<CurvePoint> path , double distance){
        ArrayList<CurvePoint> copyPath = new ArrayList<>(path);

        CurvePoint endPoint = copyPath.get(copyPath.size() - 1);
        CurvePoint startPoint = copyPath.get(copyPath.size() - 2);
        ...
        endPoint.setPoint(new CurvePoint(endPoint.x + x_component , endPoint.y + y_component));
        return copyPath;
    }
